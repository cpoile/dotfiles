...
(defcustom dumb-jump-find-rules
  '((:type "function" :supports ("ag" "grep" "rg" "git-grep") :language "elisp"
    ...
    (:type "function" :supports ("ag" "grep" "rg" "git-grep") :language "jai"
     :regex "^\\s*\\bJJJ\\s *::\\s *\(?:inline\)? *(.*).*{"
     :tests ("strings :: () {"
             "strings_2 :: (x: int, y: int)   {"
             "strings_3 :: (x: int, y: int) -> bytes: int {"))

    (:type "enum" :supports ("ag" "grep" "rg" "git-grep") :language "jai"
     :regex "^\\s*\\bJJJ\\s*::\\s*enum\\s*\{"
     :tests ("Token :: enum {"
             "AnotherEnum     ::      enum          {"))

    (:type "struct" :supports ("ag" "grep" "rg" "git-grep") :language "jai"
     :regex "^\\s*\\bJJJ\\s*::\\s*struct\\s*\{"
     :tests ("Token :: struct {"
             "AnotherEnum     ::      struct          {"))

    ;; TODO: probably fails on some other valid combinations, needs more work
    (:type "stuct-field" :supports ("ag" "grep" "rg" "git-grep") :language "jai"
     :regex ":: struct\\s*{\\n\\s*JJJ\\s*:\\s*[a-z-A-Z-_\[\]\.]+;"
     :tests ("Foo :: struct { abc: u32; test: [..] string; }"))
...
(defcustom dumb-jump-language-file-exts
  '((:language "elisp" :ext "el" :agtype "elisp" :rgtype "elisp")
    ...
    (:language "jai" :ext "jai" :agtype nil :rgtype "jai")
